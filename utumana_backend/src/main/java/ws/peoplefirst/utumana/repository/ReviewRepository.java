...
public interface ReviewRepository extends JpaRepository<Review,Long>{
    ...
    @Query("SELECT new ws.peoplefirst.utumana.dto.ReviewUserDTO(r.id, r.bookingId, r.title, r.description, r.overallRating, r.approvalTimestamp, r.comfort, r.convenience, r.position, u.name, u.profilePictureUrl) "
            + "FROM Review r "
            + "JOIN Booking b ON r.bookingId = b.id "
            + "JOIN User u ON b.userId = u.id "
            + "WHERE u.id = :userId")
    public List<ReviewUserDTO> findReviewUserById(@Param(value="userId")Long userId);
